...
**Non-F-Droid release builds are arm64-only.** The EAS `production` (store AAB) and `production-apk` (GitHub release) profiles pass `-PreactNativeArchitectures=arm64-v8a` in their `gradleCommand` (see `eas.json`). The default `reactNativeArchitectures` in `gradle.properties` still lists all four ABIs for local debug builds; keep release profiles pinned to `arm64-v8a` so the release APK doesn't carry 32-bit arm / emulator-ABI code. These release profiles also enable R8 minification and resource shrinking via `expo-build-properties` (`enableProguardInReleaseBuilds`, `enableShrinkResourcesInReleaseBuilds`); F-Droid keeps them off because its source-built Expo modules haven't been validated under `-minify`.
